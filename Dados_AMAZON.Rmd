...
Para conhecer detalhes da estrutura da *tibble* `dados`, é só usar a função `str()`:
...
Os mesmos dados diposnibilizados nos formatos `CSV` e `XLSX` se encontram também disponíveis na tabela de atributos de um `shape file` com a localização geográfica precisa de cada árvore. O *download* da pasta zipada com esse `shape file` permite criar o objeto `dadosComGeo`, necessário para desenvolver os exercícios que demandam a precisa localização geográfica das árvores.

Os exercícios com as árvores georeferenciadas usam funções do pacote **sf** do **R**, que permite ler camadas vetoriais (*layers*) de informação SIG armazenadas no formato `shape`. Use o seguinte código para acessar essa informação.
...
# Exemplo de tabela com a cinco maiores castanheiras
dadosSemGeo %>%
...
É claro que, neste caso, o mais interessante é gerar gráficos que se aproveitam do georeferenciamento dos dados. O código abaixo, por exemplo, filtra a tabela de atributos do `shape` `Arvores` para ficar apenas com as árvores de `Castanheira`. Lê os demais `shapes` e integra a localização dessas árvores com os limites das UTs e das APPs.
